...
public class PedidoControlador {
	...
	public void marcarPedido(int id) {
		if(this.modelo.get_ListaIDs().contains(id)) {
			this.modelo.MarcarPedidoCompletado(id);
			this.vista.mostrarMensaje("Pedido completado");
		}else {
			this.vista.mostrarMensaje("Error: ID no encontrado");
		}
	}
	
	//Sobrecarga del metodo mostrarPedidos
	public void mostrarPedidos(boolean estado) {
		ArrayList<Pedido> pedidosEstado = this.modelo.get_ListaPorEstado(estado);
		this.vista.mostrarPedidos(pedidosEstado);
	}
	
	//Metodo ContarPedidos
	public void contarPedidosPendientes() {
		this.vista.mostrarMensaje("Pedidos pendientes: " + 
									this.modelo.contadorPedidosPendientes());
	}
	
	public void mostrarHistorial() {
		this.vista.mostrarMensaje("--------Historial de pedidos(Eliminados-Completados)--------");
		this.vista.mostrarPedidos(this.modelo.getHistorial().get_Historial());
	}
	...
	public void iniciar() {
		...
		do {
			...
			switch (opcion) {
				...
				case "8":
					int id2 = this.vista.solicitarID();
					this.marcarPedido(id2);
					break;
					
				case "9":
					boolean estado = this.vista.solicitarEstado();
					this.mostrarPedidos(estado);
					break;
